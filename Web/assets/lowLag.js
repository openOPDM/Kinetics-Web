...
var lowLag = new function () {
	...
	this.init = function (config) {
		...
		case 'webkitAudio':
			...
			this.pause = this.pauseSoundWebkitAudio;
		...
		case 'audioTag':
			...
			this.pause = this.pauseSoundAudioTag;
			break;
	...
	this.pauseSoundSM2 = function (tag) {
		lowLag.msg("pauseSoundSM2 " + tag);
	...
	this.playSoundWebkitAudio = function (tag) {
		function waitForIt() {
			"use strict";
			if (lowLag.webkitAudioBuffers[tag]) {
				...
				lowLag.latestWebKitAudio = source;
			} else {
	...
	this.pauseSoundWebkitAudio = function (tag) {
		lowLag.latestWebKitAudio.stop(0);
	};
	...
	this.pauseSoundAudioTag = function (tag) {
		"use strict";
		lowLag.msg("pauseSoundAudioTag " + tag);
		var cloneId = "lowLagCloneElem_" + (lowLag.audioTagID - 1);
		var cloneElem = document.getElementById(cloneId);
		if (cloneElem) {
			cloneElem.currentTime = 0;
			cloneElem.volume = 0;
			cloneElem.pause();
